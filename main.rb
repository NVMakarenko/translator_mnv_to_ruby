...
def getAdder(n_rec,lexem,i)
  tableAdder=Array.new
  if n_rec=='real'then adder=lexem.to_f
     elsif n_rec=='integer' then adder=lexem.to_i
  end
  tableAdder[i]=adder
  i+=1
end
...
tableOfIdent=Array.new
tableOfSigns=Array.new
i=0
j=0
k=0
state='q0'
         ...
         (if(state=='q3') then(
            ...
            (token=getToken(state,lexem)
              if token != 'keyword' then
                  ...
                  if n_rec['id']=='real'
                    then (tableOfIdent[j]=lexem.to_f
                    j+=1)
                  elsif n_rec['id']=='integer'
                    then (tableOfIdent[j]=lexem.to_i
                    j+=1)
                  end)
            ...
            (lexem+=char
              ...
              n_rec['token']=token
              n_rec['id']=''
              if (token=='add_op'|| token=='math_op') then
                (tableOfSigns[k]=lexem.to_s
                k+=1)
              end
            fr.puts "#{n_rec}"
            lexem=''
...
puts "RPN statement is: "
j=0
while j<tableOfIdent.size do
  puts "#{tableOfIdent[j]}"
  j+=1
end
k=0
while k<tableOfSigns.size do
  puts "#{tableOfSigns[k]}"
  k+=1
end
end
